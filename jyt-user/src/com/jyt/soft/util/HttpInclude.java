 ...
 * 2、可以include远程的http内容.并且会将session_id等cookie传递至该请求
 *                       
 * @Filename: HttpInclude.java
 * @Version: 2.7.0
 * @Author: 范光洲
...
public class HttpInclude {
    ...
    private static boolean isRemoteHttpRequest(String includePath) {
        return (includePath != null)
               && ((includePath.toLowerCase().startsWith("http://")) || (includePath.toLowerCase()
                   .startsWith("https://")));
    }

    private void getLocalContent(String includePath, Writer writer) throws ServletException,
                                                                   IOException {
        ByteArrayOutputStream outputStream = new ByteArrayOutputStream(8192);
        CustomOutputHttpServletResponseWrapper customResponse = new CustomOutputHttpServletResponseWrapper(
            this.response);
        customResponse.setvalue(this.response, writer, outputStream);

        String url_path = includePath.indexOf("?") > 0 ? includePath.substring(0,
            includePath.indexOf("?")) : includePath;
        String query = includePath.indexOf("?") > 0 ? includePath.substring(includePath
            .indexOf("?") + 1) : "";
    ...
    private void getRemoteContent(String urlString, Writer writer) throws MalformedURLException,
                                                                  IOException {
        ...
        try {
            Reader reader = new InputStreamReader(input, Utils.getContentEncoding(conn,
                this.response));
    ...
    public static class CustomOutputHttpServletResponseWrapper extends HttpServletResponseWrapper {
        ...
        public CustomOutputHttpServletResponseWrapper(HttpServletResponse response) {
            super(response);
            // TODO Auto-generated constructor stub
        }

        public void setvalue(HttpServletResponse response, Writer customWriter,
